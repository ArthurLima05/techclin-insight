...
-- No public access to OAuth tokens - they should only be accessed by Edge Functions
-- This prevents any unauthorized access to sensitive Google API credentials
...
-- Secure clinic data - allow read access for access key validation but restrict sensitive fields
CREATE POLICY "Public can read clinic basic info for access key validation"
ON public.clinicas
FOR SELECT
USING (true);

-- Restrict write access to clinics
CREATE POLICY "Service role can manage clinicas"
ON public.clinicas

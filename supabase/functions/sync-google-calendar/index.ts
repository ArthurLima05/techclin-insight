...
async function createCalendarEvent(accessToken: string, agendamento: any) {
  const event = {
...
serve(async (req) => {
  ...
  try {
    ...
    console.log('Iniciando sincronização para clínica:', clinicaId);
    ...
    for (const agendamento of agendamentos || []) {
      try {
        const googleEvent = await createCalendarEvent(accessToken, agendamento);
